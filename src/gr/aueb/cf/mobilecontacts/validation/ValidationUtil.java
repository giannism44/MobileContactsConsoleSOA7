...
public class ValidationUtil {
    ...
    public static String validateDTO(MobileContactInsertDTO insertDTO){
        ...
        if (insertDTO.getPhoneNumber().length() <= 5)
            errorResponse += "Ο Τηλ. Αριθμός πρέπει μα έχει περισσότερα από πέντε σύμβολα.\n";
        if(insertDTO.getFirstname().length() < 2)
            errorResponse += "To όνομα πρέπει να περιέχει δύο ή περισσότερους χαρακτήρες.\n";
        if(insertDTO.getLastname().length() < 2)
            errorResponse += "To Επώνυμο πρέπει να περιέχει δύο ή περισσότερους χαρακτήρες.\n";
